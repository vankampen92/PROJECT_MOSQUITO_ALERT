		      ...
		      int * Event, int * x_Patch)
{
  ...
  if(Table->TOTAL_No_of_EVENTS > 1){
    ...
    n_Event_Sp = n_Event/Table->No_of_AGES;
    Sp         = n_Event%Table->No_of_AGES;
  }
  else {
    n_Event    = 0;
    n_Event_Sp = 0;
  ...
  j          = x*Table->No_of_AGES + Sp;

  assert( n_Event < Table->No_of_AGES );
  assert( n_Event_Sp == 0);
					 ...
					 Parameter_Table * Table)
{
  ...
  */
  ...
  j = Sp + Patch[x]->Patch_Connections[n_Patch]*Table->No_of_AGES;
...
void assert_Total_Population (Parameter_Table * Table, double * Y)
{
  ...
  N_Time_0 = Table->No_of_AGES * Table->No_of_INDIVIDUALS;

  if(N != N_Time_0)
    printf( "Total Community Size: %g\t S(%d) * No_of_INDIVIDUALS(%g) = %g\n",
	    N, Table->No_of_AGES, Table->INITIAL_TOTAL_POPULATION, N_Time_0);
